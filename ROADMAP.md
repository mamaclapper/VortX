...
- **0.4.0, VortX expands.** Native **debrid** built in (Real-Debrid, TorBox, Premiumize, AllDebrid keys in-app, with cached-torrent checking and a browsable debrid library, no separate configuration site); a **trust filter** that drops cam rips, dead links, and fake quality labels; keyword include and exclude filters; auto-play locked to the same release group across episodes; **in-app add-on install** on every platform; a curated **Home** of collection rails; **auto-play trailers** in our own player; **picture-in-picture**; an Apple TV **Top Shelf**; and routing **Dolby Vision** and adaptive HLS through Apple's AVPlayer pipeline so DV plays with our own controls. Plus the rest of the rebrand under the hood (its own identifiers) and the start of **VortX cloud sync** for profiles and settings.
